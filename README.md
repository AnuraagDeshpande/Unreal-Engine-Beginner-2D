Beginner 2D Game made in Unreal Engine 5 using paper 2D to create a slide scrolling character and stage from a tile map, made using blueprints. All paper assets dervied from the popular legacy fantasy pack.

Future steps:
- Add more levels
- Add music
- Experiment with different assets
#

This project focuses on setting up responsive character physics, 2D sprite handling (Paper2D), and clean input mapping.

<img width="1920" height="1029" alt="Screenshot 2026-06-06 210625" src="https://github.com/user-attachments/assets/b40696aa-0130-4c28-bfa8-21671378d9e9" />

Jumping animation

<img width="1920" height="1029" alt="Screenshot 2026-06-06 210237" src="https://github.com/user-attachments/assets/8796d277-b2f8-4756-bcac-1998652a4553" />

Moving sideways 

<img width="1920" height="1029" alt="Screenshot 2026-06-06 210257" src="https://github.com/user-attachments/assets/48502d5d-3873-4c9f-8800-bd272a27f6bc" />

Idle animation

## Controls
| Action | Keybinding |
|---|---|
| **Move Left** | `A`|
| **Move Right**| `S`|
| **Jump**      | `Spacebar` |

## 🛠️ Built With
*   **Engine:** Unreal Engine 5
*   **Tooling:** Paper2D system
